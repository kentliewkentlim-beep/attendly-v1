                  ...
                  <tr key={emp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <AvatarLightbox
                          src={(emp as any).avatarUrl ? `${(emp as any).avatarUrl}${(emp as any).avatarUrl.includes("?") ? "&" : "?"}v=${new Date(emp.updatedAt).getTime()}` : null}
                          alt={emp.name || "Avatar"}
                          caption={emp.name || undefined}
                          fallback={emp.name ? emp.name.split(' ').map(n => n[0]).join('').slice(0, 2) : "?"}
                        />
                        <div>
