...
export default function AnnouncementSheet({
  ...
  // a full reflow. `clip` prevents UI scroll WITHOUT establishing a scroll
  ...
  // The overlay + sheet. Rendered via portal to <body> so it is NOT nested
  // inside the sticky <nav> that owns the bell button. iOS Safari has a bug
  // where a `position: fixed` descendant inside a `position: sticky` ancestor
  // breaks the sticky behaviour permanently â even after the fixed element is
  // unmounted. Portaling to document.body sidesteps this entirely.
  const overlay = open ? (
    <div
